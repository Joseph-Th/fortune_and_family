...
/// Folds a deterministic structural checksum directly from the `Serialize`
/// data model instead of rendering JSON text first.
///
...
/// match. This keeps the previous guarantees — stable ordering in, stable
/// checksum out, sensitive to any visited field — while removing number
/// formatting, string escaping, and JSON framing from snapshot capture.
///
/// Serialization of gameplay observation state is infallible, so the error
/// slot exists only to satisfy the [`Serializer`] trait.
#[derive(Debug, Default)]
pub(crate) struct StructuralChecksumError;

impl std::fmt::Display for StructuralChecksumError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("structural checksum serialization cannot fail")
    }
}

impl serde::ser::StdError for StructuralChecksumError {}

impl serde::ser::Error for StructuralChecksumError {
...
impl serde::Serializer for &mut ChecksumFolder {
    type Ok = ();
    type Error = StructuralChecksumError;
...
impl serde::ser::SerializeSeq for &mut ChecksumFolder {
    type Ok = ();
    type Error = StructuralChecksumError;
...
impl serde::ser::SerializeTuple for &mut ChecksumFolder {
    type Ok = ();
    type Error = StructuralChecksumError;
...
impl serde::ser::SerializeTupleStruct for &mut ChecksumFolder {
    type Ok = ();
    type Error = StructuralChecksumError;
...
impl serde::ser::SerializeTupleVariant for &mut ChecksumFolder {
    type Ok = ();
    type Error = StructuralChecksumError;
...
impl serde::ser::SerializeMap for &mut ChecksumFolder {
    type Ok = ();
    type Error = StructuralChecksumError;
...
impl serde::ser::SerializeStruct for &mut ChecksumFolder {
    type Ok = ();
    type Error = StructuralChecksumError;
...
impl serde::ser::SerializeStructVariant for &mut ChecksumFolder {
    type Ok = ();
    type Error = StructuralChecksumError;
...
mod checksum_tests {
    ...
    #[test]
    fn equal_values_fold_identically_and_distinct_values_fold_differently() {
        let first = vec![1_i64, 2, 3];
        let second = vec![1_i64, 2, 3];
        let changed = vec![1_i64, 2, 4];

        assert_eq!(
            stable_serialized_checksum(&first),
            stable_serialized_checksum(&second)
        );
        assert_ne!(
            stable_serialized_checksum(&first),
            stable_serialized_checksum(&changed)
        );
    }

    #[test]
    fn checksums_respect_ordering_lengths_and_shapes() {
        assert_ne!(
            stable_serialized_checksum(&vec![1_i64, 2]),
            stable_serialized_checksum(&vec![2_i64, 1])
        );
        // A length prefix keeps prefix-shared sequences distinct.
        assert_ne!(
            stable_serialized_checksum(&vec![1_i64, 2]),
            stable_serialized_checksum(&vec![1_i64, 2, 3])
        );
        // String boundaries stay distinct ("ab" vs "a"+"b" as a pair).
        let left = ("ab".to_owned(), "c".to_owned());
        let right = ("a".to_owned(), "bc".to_owned());
        assert_ne!(
            stable_serialized_checksum(&left),
            stable_serialized_checksum(&right)
        );
        // Numeric width is part of the shape.
        assert_ne!(
            stable_serialized_checksum(&1_i64),
            stable_serialized_checksum(&1_i32)
        );
    }

    #[test]
    fn map_checksums_track_key_and_value_changes_deterministically() {
        let mut map = BTreeMap::new();
        map.insert(1_u32, 10_u64);
        let baseline = stable_serialized_checksum(&map);

        map.insert(2_u32, 20_u64);
        let grown = stable_serialized_checksum(&map);
        assert_ne!(baseline, grown);
        // BTreeMap iteration order is sorted, so refolding is reproducible.
        assert_eq!(grown, stable_serialized_checksum(&map));

        map.insert(2_u32, 21_u64);
        assert_ne!(grown, stable_serialized_checksum(&map));
    }
}
