...
The opportunist deliberately accepts greater operating and credit exposure than the other personas. Its growth policy spends less on maintenance, and its new lending uses shorter, higher-yield terms. Once treasury exceeds a substantial surplus threshold it may also make speculative loans to the weakest available rival even when that house is not already distressed. This is a stress strategy rather than a balance recommendation: it is intended to give the harness a credible route into repayment pressure, delinquency, litigation, labor pressure, and relationship fallout so safe automated play cannot hide those systems. The deep audit does not manufacture distressed counterparties merely to exercise that chain. Snapshots and campaign maxima distinguish delinquency/default on loans issued by the player from distress on unrelated private loans, so NPC-to-player or NPC-to-NPC defaults cannot create false coverage. If several long-horizon opportunist campaigns still fail to produce meaningful player lending and enforcement activity, the report emits a coverage warning so untested credit risk cannot be mistaken for evidence that the system is healthy.
