...
| Contracts | Scheduled supply, payment, penalties, fulfillment, attribution, breach, and termination. Breach attribution records the defendant from the first attributable miss, and partially paid penalties accumulate on the contract as recoverable breach debt capped at the contractual penalty — whether or not the missed delivery terminates it, across later fulfillment or settlement. An inactive party's termination collects whatever penalty cash it still holds first. |
| Private finance | Loans, interest accrued as one 7-day share of the 360-day year's annual charge, repayment, delinquency, default, collateral, seizure, restructuring, and repayment records. An installment that cannot cover the week's interest counts as missed, so unsustainable terms reach default instead of compounding silently. A won debt judgment discharges only what the defendant paid or collateral execution recovers; a negotiated settlement extinguishes the claim in full. |
...
| Property | Ownership, value drift with district conditions, monthly condition repair so fire damage heals instead of degrading buildings permanently, tenancy, occupancy, rent scaled by the district rent index for vacancy income and sitting tenants alike, purchase, collateral, liquidation, lien settlement, and distressed civic guarantees. Closed or insolvent occupants are evicted at the weekly settlement; a recovered firm re-occupying rented premises restores its tenancy. |
...
| AI | Deterministic objectives for property, supply, office, debt, legitimacy, liquidity, and rival pressure; monthly household upkeep with standing penalties for shortfalls; private-credit participation through the canonical loan machinery that underwrites only lifetime-profitable firms and borrows no more than the motivating shortfall; accumulation skims that never strip a firm below its recapitalization target; property purchases preserving the recovery reserve. |
| Crises | Grain, banking, fire, epidemic, guild, external-authority, and trade crises with detection, escalation, response, recovery, effects, and resolution. Trade disruptions track their route condition: organized responses heal the routes, a watch directive counts as a response, and once every route heals below the detection threshold the disruption resolves with it instead of lingering as a phantom threat. Crisis phantom demand affects prices only, never wages. |
